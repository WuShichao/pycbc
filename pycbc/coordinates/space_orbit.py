...
class LisaEqualArmOrbit(_LisaGuidingCenter, _EqualArmConstellation):
    ...
    `OrbitProvider` -- e.g. to pass to `constellation_frame`/
    `link_vector`, or as a baseline against a numeric or other-mission
    orbit. `LisaEqualArmOrbit()` with no arguments reproduces the
...
def link_vector(t, orbit, sc_emitter, sc_receiver):
    ...
    Both spacecraft are evaluated at the same time `t`, i.e. this neglects
    the light travel time between emission and reception -- unlike
    `lisaorbits.Orbits.compute_unit_vector`, which retards the emitter.
    The same-time value is what the spatial projection factors in
    single-link response formulas (e.g. n_hat . k_hat) actually need, to
    the same order those formulas are already accurate to. Applications
    needing the true retarded emission time (e.g. exact TDI time-delay
    operators) can solve for it with `t_detector_from_ssb`'s approach.
